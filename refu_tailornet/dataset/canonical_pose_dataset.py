...
import global_var
from tnutils.rotation import get_Apose
...
class ShapeStyleCanonPose(Dataset):
    ...
    def __init__(self, garment_class, gender, shape_style_list_path='avail.txt', split=None):
        ...
        # self.betas = torch.nn.parameter.Parameter(torch.from_numpy(betas.astype(np.float32)), requires_grad=False)
        # self.gammas = torch.nn.parameter.Parameter(torch.from_numpy(gammas.astype(np.float32)), requires_grad=False)
        # self.unpose_v = torch.nn.parameter.Parameter(torch.from_numpy(unpose_v.astype(np.float32)), requires_grad=False)
        # self.apose = torch.nn.parameter.Parameter(torch.from_numpy(get_Apose().astype(np.float32)), requires_grad=False)
