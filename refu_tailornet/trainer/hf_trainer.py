...
class HFModel(torch.nn.Module):
    def __init__(self,  params, ckpt = None):
...
def get_best_model(log_dir, epoch_num = None):
    ...
    if epoch_num is None:
        ...
        ckpt_path = os.path.join(ckpt_dir, "{:04d}".format(epoch_num), 'lin.pth.tar')

    model = HFModel(params, ckpt_path)
    return model
...
def parse_argument():
    ...
    parser.add_argument('--log_name', default="tn_hf_new")
