...
def plot_surface(decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite, points=None, with_points=False, latent=None, connected=False, filename_format_simple=False):

    if filename_format_simple == True:
        filename = '{0}/{1}'.format(path, shapename)
    else:
...
def get_surface_verts(points,decoder,latent,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite):
